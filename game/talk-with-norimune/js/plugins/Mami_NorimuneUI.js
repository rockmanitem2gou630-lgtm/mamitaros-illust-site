/*:
 * @target MZ
 * @plugindesc 則宗さん会話作品専用UI Ver2.0
 * @author マミタロス
 *
...
(() => {
    ...
    const pluginName = "Mami_NorimuneUI";
    ...
    class Sprite_NorimuneButton extends Sprite_Clickable {
        constructor(data) {
            super();
    ...
    function createNorimuneButtons(scene) {
        if (!scene._spriteset) {
            return;
        }

        if (scene._norimuneButtonContainer) {
            scene._spriteset.removeChild(
                scene._norimuneButtonContainer
            );
        }

        const container = new Sprite();
        scene._norimuneButtonContainer = container;
        scene._norimuneButtons = [];

        for (const data of BUTTON_DATA) {
            const button = new Sprite_NorimuneButton(data);

            container.addChild(button);
            scene._norimuneButtons.push(button);
        }
